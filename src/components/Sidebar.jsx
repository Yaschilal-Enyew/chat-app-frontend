...
const Sidebar = () => {
    ...
    <div className={` bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ""} `}>
      ...
      <div className='pb-5'>
        <div className='flex justify-between items-center'>
          ...
          <img src={assets.aura} alt='logo' className='max-w-40' />

          <div onClick={()=>setIsOptionClicked(!isOptionClicked)} className="relative py-2 group">
            <img src={assets.menu_icon} alt="Menu" className='max-h-5 cursor-pointer' />
            <div className={`${isOptionClicked ? "block" : "hidden"} absolute top-full right-0 z-20 w-32 p-5 rounded-md bg-[#282142] border border-gray-600 text-gray-100  `}>
  ...
  <div className='flex flex-col'>
     ...
     <div onClick={()=>{setSelectedUser(user); setUnseenMessages(prev=>({
      ...
      {unseenMessages[user._id] >0 && <p className='absolute top-4 text-xs right-4 h-5 w-5
      flex justify-center items-center rounded-full bg-red-500/50'>{unseenMessages[user._id]}</p>}
      </div>
    ) )}
      </div>
    </div>
